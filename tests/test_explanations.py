...
def test_cat_parse(nursery_enc):
    rule_parser = RuleParser(nursery_enc.encoder.preprocessor.get_feature_names_out().tolist())
    assert rule_parser.parse_cat(("cat__field_value", "is", 0.5)) == ("field", "is", "value")
    assert rule_parser.parse_cat(("field_name_value", "is", 0.5)) == ("field_name", "is", "value")


def test_parse_rule_cervical(cervicalb_enc):
    rule_parser = RuleParser(cervicalb_enc.encoder.preprocessor.get_feature_names_out().tolist())
