...
def stability(y_pred: np.uint8, z_pred: np.ndarray, Z: np.ndarray, pattern: tuple[NodePattern], K: np.uint8) -> float:
...
def true_negative_rate(y_pred: np.uint8, z_pred: np.ndarray, Z: np.ndarray, pattern: tuple[NodePattern]) -> float:
...
def exclusive_coverage(y_pred: np.uint8, z_pred: np.ndarray, Z: np.ndarray, pattern: tuple[NodePattern], K: np.uint8) -> float:
...
def adjusted_cardinality_weight(cardinality: np.uint8, cardinality_regularizing_weight: float):
    if cardinality_regularizing_weight < 0:
        warnings.warn("Negative values of alpha are allowed but favour shorter path segments, which reduces the importance of interation terms.")
    return (cardinality - cardinality_regularizing_weight) / cardinality
    

def pattern_importance_score(cardinality: np.uint8, support_regularizing_weight: float = 1.0, entropy_regularizing_weight: float = 1.0, cardinality_regularizing_weight: float = 1.0):
    return support_regularizing_weight * entropy_regularizing_weight * adjusted_cardinality_weight(cardinality=cardinality, cardinality_regularizing_weight=cardinality_regularizing_weight)
