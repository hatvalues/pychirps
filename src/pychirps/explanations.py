...
class RuleParser:
    ...
    def leq_segments(self):
        return [{True: "<=", False: ">"} if feature_name.startswith("num__") else {True: "is not", False: "is"} for feature_name in self.feature_names]
    ...
    def parse(self, pattern: tuple[NodePattern], y_pred: int) -> str:
        num_parse = [
            (self.feature_names[node.feature].replace("num__", ""), self.parse_leq(node), node.threshold)
            for node in pattern
        ]
        cat_parse = [self.parse_cat(node) if node[0].startswith("cat__") else node for node in num_parse]
